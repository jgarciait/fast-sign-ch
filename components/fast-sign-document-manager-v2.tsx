...
interface FastSignDocumentManagerV2Props {
  onClose: () => void
}

// Componente interno que usa React Query
function FastSignDocumentManagerV2Internal({ onClose }: FastSignDocumentManagerV2Props) {
    // React Query client
  ...
  // Siempre mostrar TODOS los documentos (sin filtros)
  ...
  useEffect(() => {
    const loadCounts = async () => {
      try {
        console.log('🔢 Loading document counts for all documents...')
        // Load active count
        const activeResult = await getFastSignDocumentsCount(false, false) // archived: false, showOnlyMyDocuments: false
        if (activeResult && typeof activeResult.totalCount === 'number') {
          console.log('📊 Active documents count:', activeResult.totalCount)
          setActiveTotalCount(activeResult.totalCount)
        }
        
        // Load archived count  
        const archivedResult = await getFastSignDocumentsCount(true, false) // archived: true, showOnlyMyDocuments: false
        if (archivedResult && typeof archivedResult.totalCount === 'number') {
          console.log('📊 Archived documents count:', archivedResult.totalCount)
    ...
    const handleCheckSignature = async (documentId: string) => {
        ...
        try {
            ...
            if ((result as any).hasSignatures) {
                // Si tiene firmas, actualizar estado
                const update = await updateFastSignDocumentStatus(documentId, "firmado")
                if ((update as any).error) {
                    toast({ title: "Error", description: (update as any).error, variant: "destructive" })
                } else {
                    toast({ title: "Estado sincronizado", description: "Marcado como Firmado." })
    ...
    const handleConfirmUnarchive = async (documentId: string) => {
        ...
        try {
            ...
            queryClient.setQueryData(
                ['infinite-documents', 'archived', searchTerm, false, 100],
                (oldData: any) => {
    ...
    const performDocumentDeletion = async (documentId: string) => {
        ...
        try {
            ...
            if ((result as any).error) {
                ...
                // Invalidate all document queries to remove the deleted document from all views
                queryClient.invalidateQueries({ 
                    queryKey: ['infinite-documents'] 
                })
                console.log('✅ Document deleted and queries invalidated successfully')
            }
        } catch (error) {
            toast({
                title: "Error",
  ...
  const VirtualizedDocuments = ({ isArchived }: { isArchived: boolean }) => {
    const {
      ...
      showUserDocs: false, // Always show all documents
      pageSize: 100
    })

    // Debug: Only log key info
    if (documents.length > 0) {
      console.log('✅ VirtualizedDocuments V2 loaded:', { documentsLength: documents.length, totalCount })
    } else if (isError) {
      console.log('❌ VirtualizedDocuments V2 error:', { isError })
    }
  ...
  return (
    <div className="space-y-4 p-4 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-sm border">
                {/* Encabezado simple - Solo muestra todos los documentos */}
...
export default function FastSignDocumentManagerV2({ onClose }: FastSignDocumentManagerV2Props) {
  return (
    <QueryProvider>
      <FastSignDocumentManagerV2Internal onClose={onClose} />
    </QueryProvider>
  )
}
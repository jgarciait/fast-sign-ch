...
export default function PdfAnnotationEditor({
  ...
  const handleAnnotationResize = (id: string, newWidth: number, newHeight: number) => {
    ...
    if (hasValidUpdate) {
      ...
      if (!internalMappingMode) {
        ...
        console.log('🗺️ Mapping mode - skipping auto-save on resize')
        setHasUnsavedChanges(true) // Still mark as changed for UI feedback
      }
      setHasUserAddedAnnotations(true) // Mark user activity to prevent annotation consolidation
    } else {
  ...
  const handleAnnotationDelete = (id: string) => {
    ...
    if (onSave) {
      // Skip auto-save in mapping mode - only save when user explicitly clicks save
      if (!internalMappingMode) {
        console.log('🔄 PDF Editor: Calling onSave with updated annotations')
        onSave(newAnnotations)
      } else {
        console.log('🗺️ Mapping mode - skipping auto-save on annotation deletion')
      }
    }
  }

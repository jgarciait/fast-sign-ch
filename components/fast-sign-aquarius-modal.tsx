...
interface FastSignAquariusModalProps {
  isOpen: boolean
  onClose: () => void
  documentId: string
  documentName: string
...
function DoctypeSelector({ 
  ...
  // Convert doctypes object to array for easier handling
  ...
  return (
    <div className="relative">
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-2 border border-gray-300 rounded-md bg-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
      >
...
export default function FastSignAquariusModal({
  isOpen,
  onClose,
  documentId,
  documentName,
  onSuccess
}: FastSignAquariusModalProps) {
  ...
  const authenticateAndFetchDoctypes = async () => {
    ...
    try {
      ...
      // Step 2: Fetch doctypes
      const doctypeResult = await getAquariusDoctypes(selectedIntegration, authResult.token!)
      if (!doctypeResult.success) {
        throw new Error(doctypeResult.error || 'Failed to fetch document types')
      }
      
      setDoctypes(doctypeResult.doctypes!)
      setStep('doctype')
    } catch (error) {
  ...
  const uploadDocument = async () => {
    try {
      ...
      // Get the signed PDF from the Fast Sign print API (now uses the same method as view-signed)
      ...
      // Upload to Aquarius using the same method as view-signed
